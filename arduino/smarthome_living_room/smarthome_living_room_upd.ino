...
#define receiver_power(x) x ? PORTD = (1<<PD4) : PORTD = ~(1<<PD4); //digitalWrite(RECEIVER_POW, x);
#define sender_power(x)   x ? PORTD = (1<<PD6) : PORTD = ~(1<<PD6); //digitalWrite(SENDER_POW, x);
